...
let
		...
		zones = lists.concatMap (addr: [
			{
				...
				value = {
					...
					stores = [{
						sqlite = {
							zone_file_path = "dyn-${addr.v4.dhcp.domainName}.zone";
							journal_file_path = "dyn-${addr.v4.dhcp.domainName}.journal";
							allow_update = true;
						};
					}];
				};
			}
			{
				...
				value = {
					...
					stores = [{
						sqlite = {
							zone_file_path = "dyn-rev-${addr.v4.dhcp.domainName}.zone";
							journal_file_path = "dyn-rev-${addr.v4.dhcp.domainName}.journal";
							allow_update = true;
						};
					}];
				};
	...
	blocklistOptions = with lib; mkOption {
		type = types.submodule {
			options = {
					...
					lists = mkOption { type = types.listOf types.path; };
	...
	sqliteStoreOptions = with lib; mkOption {
		type = types.submodule {
			options = {
				zone_file_path = mkOption { type = types.str; };
